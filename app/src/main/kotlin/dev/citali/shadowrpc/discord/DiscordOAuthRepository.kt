...
object DiscordOAuthRepository {
    ...
    suspend fun getValidAccessToken(context: Context): String? =
        withContext(Dispatchers.IO) {
            ...
            val currentToken = prefs[Prefs.DiscordTokenKey]?.trim().orEmpty()
            if (currentToken.isBlank()) {
                return@withContext null
            }

            val expiresAt = prefs[Prefs.DiscordTokenExpiresAtKey] ?: 0L
            if (expiresAt == 0L || System.currentTimeMillis() + EXPIRY_SKEW_MS < expiresAt) {
                return@withContext currentToken
            }

            val refreshToken = prefs[Prefs.DiscordRefreshTokenKey]?.trim().orEmpty()
    ...
    suspend fun clearSessionFromOtherApplication(context: Context) {
        withContext(Dispatchers.IO) {
            val prefs = context.dataStore.data.first()
            if (prefs[Prefs.DiscordTokenKey]?.isNotBlank() != true) {
                return@withContext
            }
            val issuingApplicationId = prefs[Prefs.DiscordApplicationIdKey]?.trim().orEmpty()
    ...
    suspend fun clearSession(context: Context) {
        withContext(Dispatchers.IO) {
            context.dataStore.edit { prefs ->
                prefs.remove(Prefs.DiscordTokenKey)
                prefs.remove(Prefs.DiscordApplicationIdKey)
                prefs.remove(Prefs.DiscordRefreshTokenKey)
                prefs.remove(Prefs.DiscordTokenExpiresAtKey)
                prefs.remove(Prefs.DiscordUsernameKey)
                prefs.remove(Prefs.DiscordNameKey)
                prefs.remove(Prefs.DiscordAvatarUrlKey)
            }
        }
    }
    ...
    private suspend fun storeSession(
        ...
        context.dataStore.edit { prefs ->
            prefs[Prefs.DiscordTokenKey] = session.accessToken
            prefs[Prefs.DiscordApplicationIdKey] = BuildConfig.DISCORD_APPLICATION_ID
            session.refreshToken?.takeIf { it.isNotBlank() }?.let {
                prefs[Prefs.DiscordRefreshTokenKey] = it
            }
            prefs[Prefs.DiscordTokenExpiresAtKey] = session.expiresAtMillis
            session.account?.let { account ->
                prefs[Prefs.DiscordUsernameKey] = account.username
                prefs[Prefs.DiscordNameKey] = account.displayName
                prefs[Prefs.DiscordAvatarUrlKey] = account.avatarUrl.orEmpty()
            }
        }
    }
